...
#[command(about = "Manage tasks (list, create, done, cancel, resume, etc.)")]
pub enum TaskCommand {
    /// List all tasks in the current user's overview ledger.
    ///
    /// 列出当前用户 overview 台账中的全部任务。
    #[command(
        about = "List all tasks in the current user's overview ledger.",
        long_about = None
    )]
    List(ListArgs),
    ...
    #[command(
        about = "Create a new task row in the current user's overview ledger.",
        long_about = None
    )]
    Create(CreateArgs),
    ...
    #[command(
        about = "Show the unique active task (path or count).",
        long_about = None
    )]
    Active(ActiveArgs),
    ...
    #[command(
        about = "Clear the active flag on tasks without changing their status.",
        long_about = None
    )]
    Stop(StopArgs),
    ...
    #[command(
        about = "Mark a task as Done, optionally backfilling its commit hash.",
        long_about = None
    )]
    Done(DoneArgs),
    ...
    #[command(
        about = "Mark a task as Cancelled and clear its active flag.",
        long_about = None
    )]
    Cancel(CancelArgs),
    ...
    #[command(
        about = "Resume a task, atomically making it the single active row.",
        long_about = None
    )]
    Resume(ResumeArgs),
}
...
#[derive(Debug, Args)]
#[command(
    about = "List tasks with optional status filter and JSON output.",
    long_about = None
)]
pub struct ListArgs {
    #[arg(long)]
    ...
    #[arg(
        long,
        help = "Emit one JSON object per task line instead of the default TSV.",
        long_help = None
    )]
    json: bool,
}
...
#[derive(Debug, Args)]
#[command(
    about = "Stop active tasks for the current user.",
    long_about = None
)]
pub struct StopArgs {
    ...
    #[arg(
        long,
        help = "Stop every active task for the current user.",
        long_help = None
    )]
    all: bool,
}
...
#[derive(Debug, Args)]
#[command(
    about = "Mark a task as Done with optional commit hash.",
    long_about = None
)]
pub struct DoneArgs {
    ...
    #[arg(
        long,
        value_name = "HASH",
        help = "Commit hash to backfill into the row.",
        long_help = None
    )]
...
#[derive(Debug, Args)]
#[command(
    about = "Cancel a task without shipping a commit.",
    long_about = None
)]
...
#[derive(Debug, Args)]
#[command(
    about = "Resume a task, making it the single active row.",
    long_about = None
)]
pub struct ResumeArgs {
    ...
    #[arg(
        long = "task-id",
        value_name = "ID",
        help = "Task id to resume; required.",
        long_help = None
    )]
    task_id: String,
}
...
pub struct ActiveArgs {
    ...
    #[arg(
        long,
        help = "Print the number of active tasks instead of the path.",
        long_help = None
    )]
    count: bool,
    ...
    #[arg(
        long,
        help = "Print the active task file path.",
        long_help = None
    )]
    path: bool,
}
...
#[derive(Args, Debug)]
#[command(
    about = "Create a new task row with optional title, commit, and mode.",
    long_about = None
)]
pub struct CreateArgs {
    ...
    #[arg(
        value_name = "TITLE",
        help = "Positional task title (mutually exclusive with --title).",
        long_help = None
    )]
    ...
    #[arg(
        value_name = "COMMIT",
        help = "Positional commit hash (mutually exclusive with --commit).",
        long_help = None
    )]
    ...
    #[arg(
        long,
        value_name = "TITLE",
        help = "Task title (mutually exclusive with the positional form).",
        long_help = None
    )]
    title: Option<String>,
    ...
    #[arg(
        long,
        value_name = "COMMIT",
        help = "Commit hash to record on the new row.",
        long_help = None
    )]
    commit: Option<String>,
    ...
    #[arg(
        ...
        help = "Atomically clear existing active rows and create with active=true.",
        long_help = None
    )]
    switch: bool,
    ...
    #[arg(
        ...
        help = "Create with active=false, leaving In-Progress active rows untouched.",
        long_help = None
    )]
    inactive: bool,
    ...
    #[arg(
        long = "allow-default",
        help = "Bypass the default workspace collaborator guard.",
        long_help = None
    )]
    allow_default: bool,
}
    ...
    if args.json {
        for task in tasks {
            let line = serde_json::to_string(&task)
                .with_context(|| format!("failed to serialize task, task_id: {}", task.task_id))?;
            println!("{line}");
        }
    } else {
        for task in tasks {
...
pub fn create_task(args: CreateArgs) -> Result<()> {
    ...
    let line = serde_json::to_string(&new_row).context("failed to serialize new task")?;
    println!("{line}");
    Ok(())
}
...
pub fn done_task(args: DoneArgs) -> Result<()> {
    ...
    let line = serde_json::to_string(&updated).context("failed to serialize updated task")?;
    println!("{line}");
    Ok(())
}
