...
/// 决定 init 末尾是否启用 VuePress 集成。
///
/// 决策树：`--enable-vuepress` flag 显式启用 → `true`；否则 stdin 是
/// TTY → 走交互询问（默认 No）；非 TTY 且无 flag → `false`（CI 安全
/// 默认）。
///
/// Decides whether init should install VuePress at the end. `--enable-
/// vuepress` always wins; otherwise prompt on a TTY (default No);
/// silently skip on a non-TTY so unattended runs don't deadlock.
fn resolve_enable_vuepress(flag: bool) -> bool {
    ...
    prompt_yes_no(
        "是否启用 VuePress（在浏览器中预览任务文件）？\nEnable VuePress (browser preview for task files)?",
        false,
    )
}
...
///
/// Bilingual TTY yes/no prompt. Accepts `y`/`yes` / `n`/`no` (any
/// case); falls back to `default` for empty / unrecognized input and
/// any IO error so callers can keep this synchronous and infallible.
