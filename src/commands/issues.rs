...
pub enum IssuesCommand {
    ...
    #[command(
        about = "Print all promoted issues as Markdown on stdout",
        long_about = None
    )]
    List,
    ...
    #[command(
        about = "Print issues relevant to the given changed files or keywords",
        long_about = None
    )]
    ...
    #[command(
        about = "Append promoted issues from stdin JSONL into .hotpot/issues.jsonl",
        long_about = None
    )]
    Promote,
    ...
    #[command(
        about = "Manage project-shared temporary issue candidates",
        long_about = None
    )]
    Candidate {
        #[command(subcommand)]
...
pub enum CandidateCommand {
    ...
    #[command(
        about = "Print all project-shared temporary candidates as JSONL on stdout",
        long_about = None
    )]
    List,
    ...
    /// `.hotpot/issue-candidates.jsonl`。
    #[command(
        about = "Append user-confirmed candidates from stdin JSONL into .hotpot/issue-candidates.jsonl",
        long_about = None
    )]
    Add,
    ...
    /// 在晋升或主动丢弃完成后清空候选文件。
    #[command(
        about = "Truncate the candidates file after promotion or deliberate discard",
        long_about = None
    )]
    Clear,
}
...
pub struct RelevantArgs {
    ...
    #[arg(
        long = "changed-file",
        value_name = "PATH",
        help = "Path of a changed file used to rank relevant issues; may be repeated",
        long_help = None
    )]
    ...
    #[arg(
        long = "keyword",
        value_name = "WORD",
        help = "Free-form keyword to match against issue content; may be repeated",
        long_help = None
    )]
    keywords: Vec<String>,
    ...
    #[arg(
        long,
        default_value_t = 5,
        help = "Maximum number of relevant issues to return",
        long_help = None
    )]
    limit: usize,
}
...
pub fn promote_issues() -> Result<()> {
    ...
    for (index, line_result) in stdin.lock().lines().enumerate() {
        let line =
            line_result.with_context(|| format!("failed to read stdin line {}", index + 1))?;
        ...
        let issue: issues::Issue = serde_json::from_str(trimmed).with_context(|| {
            format!(
                "failed to parse stdin line {} as Issue, raw content: {}",
                index + 1,
                trimmed
            )
        })?;
    ...
    if !saw_any_line {
        bail!("stdin has no issue content. Expected one Issue per line as JSONL.");
    }
...
pub fn add_candidates() -> Result<()> {
    ...
    for (index, line_result) in stdin.lock().lines().enumerate() {
        ...
        let candidate: issues::IssueCandidate =
            serde_json::from_str(trimmed).with_context(|| {
                format!(
                    "failed to parse stdin line {} as IssueCandidate, raw content: {}",
                    index + 1,
                    trimmed
                )
            })?;
    ...
    if !saw_any_line {
        bail!(
            "stdin has no issue candidate content. Expected one IssueCandidate per line as JSONL."
        );
    }
