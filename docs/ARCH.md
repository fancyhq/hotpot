...
- Outputs in code stay English (per `AGENTS.md`); natural-language responses follow `.hotpot/config.toml::language`. The shared directive lives in `assets/prompts/output-language.md` and is referenced by every main workflow prompt (`hotpot-new.md` / `hotpot-execute.md` / `hotpot-finish-work.md` / `tdd-protocol.md`) via `@.hotpot/prompts/output-language.md` (Claude/OpenCode) or `$ROOT_DIR/.hotpot/prompts/output-language.md` (Codex/Pi — the thin shells map the token explicitly). Structural anchors (CLI flags, JSON keys, `ACTIVE_CONFLICT:`, markdown section headings, `tdd: true|false`, kebab-case slugs) MUST stay English regardless of the configured language.
