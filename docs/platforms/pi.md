...
- Inlining `$ARGUMENTS` inside a prose sentence as a soft hint (for example, "treat it as the initial task idea if non-empty") is not reliable: a downstream shared workflow body that issues a strong directive such as "ask clarifying questions" can override the soft hint and cause the model to ignore the substituted text entirely. Hotpot Pi templates (e.g. `hotpot-new.md`) inject command arguments with a three-part pattern: an explicit delimiter block (such as `<<< INITIAL TASK IDEA (verbatim from \`/hotpot-new\` arguments) >>> ... <<< END INITIAL TASK IDEA >>>`), an unconditional directive declaring that the block IS the user's input, and a mandatory anti-question sentence forbidding the model from asking again for the same input. The purpose of this pattern is to make the substituted text read as a data payload rather than an optional hint, so the model cannot regress to the shared workflow's default clarifying-question behavior. When the template variable can be empty (for example, when the user invokes the command with no arguments), the empty-arguments fallback must be written as an explicit `**Exception (empty arguments)**` override that names the previous directive and gives an objective empty-detection criterion (such as "no non-whitespace text between the markers"); otherwise the unconditional directive and the fallback compete in the model's attention and the model may parse the marker labels themselves as the user's input. When the Pi project loads competing system context (such as `AGENTS.md`, `CLAUDE.md`, or a global skills list), even a strong delimiter-block-plus-Exception body can be silently absorbed as background documentation, causing the model to greet with "What would you like me to do?" instead of starting the workflow. In that case, the prompt template MUST open (immediately after the frontmatter) with a user-voice framing block that names this prompt as the user's active request and explicitly negates the wrong responses; Hotpot's `hotpot-new.md` uses an `=== USER ACTIVE REQUEST === ... === END USER ACTIVE REQUEST ===` block for this.
