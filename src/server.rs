...
fn ensure_brainstorm_session_dir(session_dir: &Path) -> Result<()> {
    ...
    let session_dir = session_dir.canonicalize().with_context(|| {
        format!(
...
fn stop_brainstorm_sessions(root_dir: &str) -> Result<()> {
    ...
    for entry in fs::read_dir(&brainstorm_dir)
        .with_context(|| format!("failed to read directory: {}", brainstorm_dir.display()))?
    {
...
fn prepare_session_dirs(session_dir: &Path) -> Result<()> {
    fs::create_dir_all(content_dir(session_dir)).with_context(|| {
        format!(
            "failed to create content dir: {}",
            content_dir(session_dir).display()
        )
    })?;
    fs::create_dir_all(state_dir(session_dir)).with_context(|| {
        format!(
            "failed to create state dir: {}",
            state_dir(session_dir).display()
        )
    })?;
    Ok(())
}
...
fn start_daemon(session_dir: &Path, host: &str, url_host: &str, port: u16) -> Result<()> {
    let exe = std::env::current_exe().context("failed to get current hotpot executable path")?;
    Command::new(exe)
        .arg("server")
        .arg("serve")
        .arg("--session-dir")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to start hotpot server background process")?;
    Ok(())
}
    ...
    for _ in 0..100 {
        if path.exists() {
            return fs::read_to_string(&path)
                .with_context(|| format!("failed to read server-info: {}", path.display()));
        }
...
fn write_server_info(options: &ServeOptions) -> Result<ServerInfo> {
    ...
    fs::write(state_dir.join("server.pid"), info.pid.to_string())
        .with_context(|| format!("failed to write server.pid: {}", state_dir.display()))?;
    fs::write(
        state_dir.join("server-info"),
        format!("{}\n", serde_json::to_string(&info)?),
    )
    .with_context(|| format!("failed to write server-info: {}", state_dir.display()))?;
    Ok(info)
}
    ...
    match (request.method(), request.url()) {
        ...
        (&Method::Post, "/event") => {
            ...
            request
                ...
                .context("failed to read event request body")?;
    ...
    FTerminate: Fn(u32) -> Result<()>,
{
    ...
    if let Err(err) = terminate_pid_fn(pid) {
        ...
        return Err(err).context(format!("failed to stop server process, pid: {pid}"));
    }
...
fn terminate_pid(pid: u32) -> Result<()> {
    #[cfg(unix)]
    {
        let status = Command::new("kill")
            .arg(pid.to_string())
            .status()
            .with_context(|| format!("failed to stop server process, pid: {pid}"))?;
        if !status.success() {
...
fn available_port(host: &str) -> Result<u16> {
    let listener = TcpListener::bind(format!("{host}:0"))
        .with_context(|| format!("failed to bind temporary port, host: {host}"))?;
