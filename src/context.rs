//! Hotpot runtime context resolution.
//!
//! Shared context resolution for hooks and business commands. Business
//! commands (`Context::resolve` / `resolve_root_dir`) use an env-first
//! chain: `ROOT_DIR` → cwd → canonicalize. Hook commands
//! (`Context::from_payload`) use a payload-first chain: payload cwd → cwd
//! fallback → canonicalize, and intentionally ignore ambient `ROOT_DIR`
//! so hook output reflects the platform-provided cwd.
//!
//! Hotpot 运行时上下文解析。
//!
...
/// Resolved Hotpot runtime context shared by hooks and business commands.
///
/// All `#[serde(rename = "...")]` field names are public contract:
/// OpenCode's `bash-before.ts` and Pi's `extensions/hotpot/index.ts`
/// read these uppercase snake-case JSON keys directly; the literals must
/// not change.
///
/// The three VuePress fields (`HOTPOT_VUEPRESS_ENABLED` /
/// `HOTPOT_VUEPRESS_PORT` / `HOTPOT_VUEPRESS_URL`) are gated by
/// `.hotpot/config.toml::[vuepress] enabled`. Disabled projects always
/// serialize `HOTPOT_VUEPRESS_ENABLED = "false"`; the other two fields
/// are empty and skipped from JSON.
///
...
pub struct Context {
    ...
    /// Hotpot username used by the current session.
    ///
    /// 当前会话使用的 Hotpot 用户名。
    ...
    /// Shared body prompt path for the `/hotpot:new` workflow.
    ///
    ...
    /// Shared body prompt path for the `/hotpot:execute` workflow.
    ///
    ...
    /// Shared body prompt path for the `/hotpot:finish-work` workflow.
    ///
...
/// `"English"`，绝不报错——该函数运行在每一轮 hook 上，必须无声。
...
/// Builds a complete [`Context`] from an already-resolved root directory.
///
...
fn build_context(root_dir: String) -> Result<Context> {
    ...
    // enabled 始终输出作为 hook/bootstrap 公共契约；port/url 仅启用态填充。
...
/// Returns the project-local absolute path for a named Hotpot prompt.
///
/// Prompt files are installed by `hotpot init` from repository assets
/// into `.hotpot/prompts/<name>.md`, keeping them under the Hotpot
/// namespace instead of the project root.
///
...
/// Runs a git command in `root_dir` and returns trimmed stdout when present.
///
...
mod tests {
    ...
    //!
    //! `resolve_language*` 单元测试；env-mutating 测试通过 `env_lock()` 串行化。
    ...
    /// Writes raw contents to `<root>/.hotpot/config.toml`.
    ///
    ...
    fn resolves_language_from_env_override() {
        ...
        // No config.toml; env alone should win.
    ...
    fn commented_out_default_yields_english() {
        ...
        // Simulate the stock hotpot-config.toml template with language commented out.
    ...
    fn corrupt_toml_yields_english_no_panic() {
        ...
        // Write malformed TOML and verify parser None falls back to English.
        // 故意写入未闭合引号的 TOML，确认解析器 None 之后回退到 English。
    ...
    fn missing_file_yields_english() {
        ...
        // No config.toml; run resolver against the default path.
        // 不写 config.toml，跑解析。
        let (lang, source) = resolve_language_with_source(&root.path().display().to_string());
        assert_eq!(lang, "English");
        assert_eq!(source, LanguageSource::Default);
    ...
    fn vuepress_enabled_defaults_to_false() {
        ...
        // No env and no config.toml should fall back to literal false.
    ...
    fn vuepress_enabled_garbage_env_falls_through() {
        ...
        // Simulate an invalid enabled value; it should be treated as None
        // and fall through to default false because config also lacks it.
        // 模拟用户把 enabled 写成 "yes"，不在 {"true","false"} 集合内。
    ...
    fn vuepress_port_out_of_range_falls_through() {
        ...
        // 70000 is outside u16 range and should fall through to default.
        // 70000 超出 u16 范围，应当被解析器拒绝并继续走链路。
        write_config(root.path(), "[vuepress]\nport = 70000\n");
