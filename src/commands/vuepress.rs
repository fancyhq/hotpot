...
//!
//! CLI shell for the `hotpot vuepress` family. Splits into asset
//! lifecycle (install / uninstall) and service lifecycle (start / stop
//! / status); real work lives in [`crate::vuepress`]. Port resolution
//! mirrors [`crate::context::resolve_vuepress_port`] so the value the
//! hook layer injects matches what `start` actually binds.
