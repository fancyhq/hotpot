...
- **轻量 hook prompt 契约（模型可见上下文）**：`PreToolUse` 的模型可见 hook 输出现在只带 `ROOT_DIR`、`HOTPOT_LANGUAGE` 和简短语言指令，由 `src/commands/hook.rs::prompt_context_message` 组装。`HOTPOT_NEW_PROMPT`、`HOTPOT_EXECUTE_PROMPT` 等路径字段不列入——模型通过 `$ROOT_DIR/.hotpot/prompts/<name>.md` 拼接定位。Claude 和 Codex 的 `PreToolUse` hook 都在 `Edit|Write` 及每次 `Bash` 调用前触发，因为平台 matcher 只能按工具名过滤，不能按 Bash 命令内容过滤；轻量 payload 让这种更宽的触发范围对 Hotpot Bash 命令来说可接受。不配置、不处理按用户消息触发的 prompt hook。`SubagentStart` / `SessionStart` 这类 review-memory hook 仍使用 `context_lines` 输出完整上下文给 review agent。`hotpot hook bootstrap`（`--format json`／`--format shell`）继续全量输出所有字段——给 shell/plugin 使用的完整 env 契约保持不变。
...
- 代码里的输出保持英文（见 `AGENTS.md`）；自然语言对外回复按 `.hotpot/config.toml::language` 决定。解析逻辑在 Rust（`src/context.rs::resolve_language[_with_source]`，链路：env `HOTPOT_LANGUAGE` → `<root>/.hotpot/config.toml` 顶层 `language` → `"English"`）。每个平台 hook 都会重新注入语言指令，对抗"一次指令长会话漂移"：Claude 走 `PreToolUse` + `SubagentStart`；Codex 走 `PreToolUse` + `SessionStart`；OpenCode 走插件 `shell.env` 加编排 prompt 文案；Pi 走 `pi.on("context", …)`。`src/commands/hook.rs::language_directive_message` 是简短指令的唯一来源，现通过 `prompt_context_message` 轻量 helper 内嵌到 `PreToolUse` hook 中；详细规范仍在 `assets/prompts/output-language.md`，被四份主工作流 prompt（`hotpot-new.md` / `hotpot-execute.md` / `hotpot-finish-work.md` / `tdd-protocol.md`）通过 `@.hotpot/prompts/output-language.md`（Claude/OpenCode）或 `$ROOT_DIR/.hotpot/prompts/output-language.md`（Codex/Pi）引用。结构性锚点（CLI flag、JSON 字段、`ACTIVE_CONFLICT:`、markdown 章节标题、`tdd: true|false`、kebab-case slug）无论 language 取何值都**必须保持英文**。OpenCode 的完整 bootstrap JSON 只写入 `shell.env` 给 Bash/tool 进程使用，不展开到模型可见上下文。
...
- 启用 VuePress 时，`/hotpot:new` 必须先通过 opt-in prompt 的文件存在 gate 判断状态，并在写任务文件前读取 `vuepress-style.md`。第一次创建文件就必须是最终 VuePress 格式任务；不要先创建普通 Markdown，再二次改写成 VuePress 格式。
