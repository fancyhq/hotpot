...
fn ensure_issues_exists(root_dir: &str) -> Result<PathBuf> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("failed to create issue directory: {}", path.display()))?;
    }
    ...
    fs::OpenOptions::new()
        ...
        .append(true)
        .open(&path)
        .with_context(|| format!("failed to create issues file: {}", path.display()))?;
...
pub fn ensure_issue_candidates_exists(root_dir: &str, username: &str) -> Result<PathBuf> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).with_context(|| {
            format!(
                "failed to create issue candidates directory: {}",
                path.display()
            )
        })?;
    }
    ...
    fs::OpenOptions::new()
        ...
        .append(true)
        .open(&path)
        .with_context(|| format!("failed to create issue candidates file: {}", path.display()))?;
...
fn migrate_legacy_issue_candidates(root_dir: &str, global_path: &PathBuf) -> Result<()> {
    ...
    for entry in fs::read_dir(&workspaces_dir).with_context(|| {
        ...
        let content = fs::read_to_string(&legacy_path).with_context(|| {
            format!(
                "failed to read legacy issue candidates file: {}",
                legacy_path.display()
            )
        })?;
    ...
    for (legacy_path, lines) in candidates {
        ...
        if let Err(err) = fs::rename(&legacy_path, &staging_path) {
            for (restored_legacy_path, restored_staging_path, _) in &migrations {
                if restored_staging_path.is_file() && !restored_legacy_path.exists() {
                    fs::rename(restored_staging_path, restored_legacy_path).with_context(|| {
                        format!(
                            "failed to restore legacy issue candidates file: {}",
                            legacy_path.display()
                        )
                    })?;
                }
            ...
            return Err(err).with_context(|| {
                format!(
                    "failed to stage legacy issue candidates file: {}",
                    legacy_path.display()
                )
            });
        }
    ...
    let original_len = fs::metadata(global_path)
        .with_context(|| {
            format!(
                "failed to read issue candidates file metadata: {}",
                global_path.display()
            )
        })?
        .len();
    let append_result = (|| -> Result<()> {
        let mut file = fs::OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "failed to open issue candidates file: {}",
                    global_path.display()
                )
            })?;
        ...
        for (_, _, lines) in &migrations {
            for line in lines {
                file.write_all(line.as_bytes()).with_context(|| {
                    format!(
                        "failed to write issue candidates file: {}",
                        global_path.display()
                    )
                })?;
                file.write_all(b"\n").with_context(|| {
        ...
        file.flush().with_context(|| {
            format!(
                "failed to flush issue candidates file: {}",
                global_path.display()
            )
        })
    })();
    ...
    for (legacy_path, staging_path, _) in migrations {
        ...
        if let Err(err) = fs::rename(&staging_path, &done_path) {
            ...
            return Err(err).with_context(|| {
                format!(
                    "failed to mark legacy issue candidates file as migrated: {}",
                    legacy_path.display()
                )
            });
        }
    ...
    for (legacy_path, _, done_path) in committed_migrations {
        fs::write(&legacy_path, "").with_context(|| {
            format!(
                "failed to recreate empty legacy issue candidates file: {}",
                legacy_path.display()
            )
        })?;
        fs::remove_file(&done_path).with_context(|| {
            format!(
                "failed to clean up migrated legacy issue candidates file: {}",
                done_path.display()
            )
        })?;
    }
...
fn legacy_unique_staging_path(legacy_path: &Path) -> PathBuf {
...
fn legacy_unique_done_path(legacy_path: &Path) -> PathBuf {
    legacy_path.with_file_name(format!(
...
fn restore_orphaned_legacy_staging_files(
    ...
    for entry in fs::read_dir(workspaces_dir).with_context(|| {
        ...
        for workspace_entry in fs::read_dir(&workspace_path).with_context(|| {
            format!(
            ...
            if missing_lines.is_empty() {
                fs::remove_file(&staging_path).with_context(|| {
                    format!(
                        "failed to clean up legacy issue candidates staging file: {}",
...
fn merge_staging_into_legacy(
    ...
    let mut legacy_file = fs::OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "failed to open legacy issue candidates file: {}",
                legacy_path.display()
            )
        })?;
    let original_len = legacy_file
        ...
        .with_context(|| {
            format!(
                "failed to read legacy issue candidates file metadata: {}",
                legacy_path.display()
            )
        })?
        .len();
    let append_result = (|| -> Result<()> {
        ...
        legacy_file.flush().with_context(|| {
            format!(
                "failed to restore legacy issue candidates file: {}",
                legacy_path.display()
            )
    ...
    if let Err(err) = append_result {
        fs::OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "failed to roll back legacy issue candidates file: {}",
                    legacy_path.display()
                )
            })?;
        return Err(err);
...
fn read_non_empty_lines(path: &PathBuf) -> Result<Vec<String>> {
    let content = fs::read_to_string(path)
        .with_context(|| format!("failed to read issue candidates file: {}", path.display()))?;
    Ok(content
        .lines()
        .filter_map(|line| {
            let trimmed = line.trim();
...
fn write_missing_separator(path: &PathBuf, file: &mut fs::File) -> Result<()> {
    ...
    file.write_all(b"\n")
        .with_context(|| format!("failed to write issue candidates file: {}", path.display()))
}
...
fn file_has_trailing_newline(path: &PathBuf) -> Result<bool> {
    let mut file = fs::OpenOptions::new()
        .read(true)
        .open(path)
        .with_context(|| format!("failed to open issue candidates file: {}", path.display()))?;
    let len = file
        .metadata()
        .with_context(|| {
            format!(
...
fn rollback_global_append(global_path: &PathBuf, original_len: u64) -> Result<()> {
    ...
    fs::OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "failed to roll back issue candidates file: {}",
                global_path.display()
            )
        })
}
...
pub fn get_issues_list(root_dir: &str) -> Result<Vec<Issue>> {
    ...
    let issues_content = fs::read_to_string(&issues_file_path)
        .with_context(|| format!("failed to read issues file: {}", issues_file_path.display()))?;
    ...
    issues_content
        ...
        .map(|(index, line)| {
            serde_json::from_str::<Issue>(line).with_context(|| {
                format!(
                    "failed to parse issues.jsonl line {} in file: {}",
                    index + 1,
...
pub fn get_issue_candidates_list(root_dir: &str, username: &str) -> Result<Vec<IssueCandidate>> {
    ...
    let candidates_content = fs::read_to_string(&candidates_file_path).with_context(|| {
        format!(
            "failed to read issue candidates file: {}",
    ...
    candidates_content
        ...
        .map(|(index, line)| {
            serde_json::from_str::<IssueCandidate>(line).with_context(|| {
                format!(
                    "failed to parse issue-candidates.jsonl line {} in file: {}",
                    index + 1,
                    candidates_file_path.display()
                )
            })
...
pub fn append_issue_candidate(
    ...
    with_file_lock(&candidates_file_path, || {
        let mut line =
            serde_json::to_string(candidate).context("failed to serialize issue candidate")?;
        line.push('\n');
...
pub fn append_issue(root_dir: &str, issue: &Issue) -> Result<()> {
    ...
    with_file_lock(&issues_file_path, || {
        let mut line = serde_json::to_string(issue).context("failed to serialize issue")?;
        line.push('\n');

        fs::OpenOptions::new()
            .append(true)
            .open(&issues_file_path)
            .with_context(|| format!("failed to open issues file: {}", issues_file_path.display()))?
            .write_all(line.as_bytes())
            .with_context(|| {
                format!(
                    "failed to write issues file: {}",
                    issues_file_path.display()
                )
            })
    })
...
pub fn clear_issue_candidates(root_dir: &str, username: &str) -> Result<()> {
    ...
    with_file_lock(&candidates_file_path, || {
        fs::write(&candidates_file_path, "").with_context(|| {
            format!(
                "failed to clear issue candidates file: {}",
                candidates_file_path.display()
            )
        })
    })
...
mod tests {
    ...
    fn migrates_legacy_workspace_candidates_to_global_file() {
        ...
        let root_dir = root.path().display().to_string();
        let alice_legacy = root
            .path()
            .join(".hotpot/workspaces/alice/issue-candidates.jsonl");
        let bob_legacy = root
            .path()
            .join(".hotpot/workspaces/bob/issue-candidates.jsonl");
    ...
    fn failed_global_append_keeps_legacy_candidates() {
        ...
        assert!(
            format!("{err:#}").contains("failed to open issue candidates file"),
