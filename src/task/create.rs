...
fn create_task_locked(
    ...
    if username == "default" && !live_active.is_empty() && !is_default_allowed(allow_default) {
        let listed = live_active
            ...
            .join(", ");
        bail!(
            "ACTIVE_CONFLICT: refusing to share \"default\" workspace with another collaborator (In-Progress task: {listed}). \
             Run `hotpot update --username <your-name>` to set your own identity; \
             for single-user projects, set HOTPOT_ALLOW_DEFAULT_USERNAME=1 or pass `--allow-default`."
        );
    }
    ...
    if mode == CreateMode::Default && !live_active.is_empty() {
        let listed = live_active
            ...
            .join(", ");
        bail!(
            "ACTIVE_CONFLICT: found {n} active=true and In-Progress tasks: {listed}. \
             Use `--switch` to shift execution focus; use `--inactive` to record without switching.",
            n = live_active.len()
        );
    }
