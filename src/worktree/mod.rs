...
///
/// Default worktree physical directory (relative to project root).
///
...
fn read_configured_base_dir(root_dir: &str) -> Result<Option<String>> {
    ...
    let doc: toml_edit::DocumentMut = raw
        .parse()
        .with_context(|| format!("failed to parse {} (TOML syntax error)", cfg_path.display()))?;
    let value = doc
        .get("worktree")
...
fn current_branch(cwd: &Path) -> Result<String> {
    ...
    if !output.status.success() {
        bail!(
            "`git rev-parse --abbrev-ref HEAD` exited with non-zero: {}",
    ...
    if name == "HEAD" {
        bail!(
            "HEAD is in a detached state, cannot record worktree base branch. \
...
pub fn create_worktree(root_dir: &str, username: &str, task_id: &str) -> Result<WorktreeInfo> {
    let task_row = task::get_task_by_id(root_dir, username, task_id)?
        .ok_or_else(|| anyhow::anyhow!("task_id = {task_id} not found"))?;
    ...
    if !output.status.success() {
        bail!(
            "`git worktree add -b {branch} {path}` failed: {stderr}",
...
pub fn remove_worktree(
    ...
    if !path.is_empty() {
        ...
        if !output.status.success() && !force {
            bail!(
                "`git worktree remove {path}` failed: {stderr}\nUse --force for forceful cleanup.",
                stderr = String::from_utf8_lossy(&output.stderr).trim()
            );
        }
    }
