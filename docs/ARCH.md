...
`hotpot update` uses the same asset engine as `hotpot init`, but it only refreshes platforms already present on disk. Its report intentionally distinguishes the asset classes below so operators can see exactly where `--force` applies:

| Strategy | Representative paths | Update behavior | `--force` behavior |
|---|---|---|---|
| `Owned` | `.hotpot/prompts/*.md`, platform Hotpot-private agents/commands/skills/plugins/extensions, and VuePress opt-in prompts when VuePress is enabled | Rewrites missing or matching Hotpot-private templates; aborts on differing existing files unless `--force` is set. | Overwrites only differing `Owned` files with bundled Hotpot templates. |
| `MergeJson` / `MergeToml` / `MergeText` | `.claude/settings.json`, `.opencode/package.json`, `.codex/config.toml`, `.pi/package.json`, `.gitignore` | Idempotently merges Hotpot-managed keys or anchored blocks while preserving user-authored content. | Does not change merge into whole-file overwrite. |
| `CreateIfMissing` | `.hotpot/config.toml` | Creates the user-owned seed only when missing; skips existing files. | Does not overwrite existing seed files. |
| VuePress hub boundary | `.hotpot-hub/` | Not refreshed by `hotpot update`; hub repair requires explicit `hotpot vuepress install --force`. | Does not overwrite hub files. |

The `asset_strategy` field in `hotpot update --json` mirrors this matrix with stable English keys and includes `force_overwrites_existing` booleans for machine checks.
...
The CLI stop path is deliberately stronger than a single runtime pid kill. On Unix, `start` calls `setsid()`, so an owned runtime pid is cleaned by targeting the runtime pid's process group first and then falling back to the pid; on Windows it preserves `taskkill /T` process tree semantics. Before killing any runtime pid target, `stop` / `status` perform a runtime pid ownership check: the pid's command line must contain this exact `.hotpot-hub` path and look like a VuePress/Vite/pnpm/docs:dev command. If that check fails, Hotpot treats the runtime state as stale, does not kill that pid, keeps the runtime port fallback, and deletes `runtime.json`. Dead runtime pids and TTL expiry reuse the same cleanup path. If the parent pid is gone or untrusted but the runtime port is still held by a command line that is identifiable as this hub's VuePress/Vite/pnpm process, the runtime port fallback terminates only that Hotpot-owned process and then deletes `runtime.json`.
