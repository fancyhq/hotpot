...
mod tests {
    ...
    fn test_create_task() {
        ...
        println!("now have {} tasks", list.len());
        println!("new task row: {new_row:#?}");
        println!("last task: {:#?}", list.last().unwrap());
    }
    ...
    fn test_create_default_bails_on_in_progress_conflict() {
        ...
        assert!(
            ...
            "error message should start with ACTIVE_CONFLICT:, actual: {msg}"
        );
    ...
    fn test_get_active_task_filepath_rejects_multi_active() {
        ...
        let msg = format!("{err}");
        assert!(msg.contains("Found 2 active=true tasks"), "actual: {msg}");
    }
}
