...
/// Ensure overview.jsonl exists, creating it (and its parent directory) if missing.
///
/// Returns the file path for chained use.
///
...
/// Read all tasks from the overview ledger.
///
...
pub fn get_task_list(root_dir: &str, username: &str) -> Result<Vec<TaskInfo>> {
    ...
    let overview_content = fs::read_to_string(&task_file_path).with_context(|| {
        format!(
            "failed to read task file; file may be missing or unreadable: {}",
    ...
    overview_content
        ...
        .map(|(index, line)| {
            serde_json::from_str::<TaskInfo>(line).with_context(|| {
                format!(
                    "failed to parse overview.jsonl line {} in file: {}",
                    index + 1,
                    task_file_path.display()
                )
            })
...
/// Count active tasks in the overview ledger (used to enforce the
/// single-active invariant before creating a new task).
///
/// 获取当前 overview 中 active 为 true 的任务总数；创建新任务前应校验
/// 该值小于 1。
    ...
    F: FnMut(&mut TaskInfo),
{
    ...
    for task in &task_list {
        let line = serde_json::to_string(task)
            .with_context(|| format!("failed to serialize task info, task_id: {}", task.task_id))?;
    ...
    fs::write(&tmp_path, content).with_context(|| {
        format!(
            "failed to write temporary task file: {}",
            tmp_path.display()
        )
    })?;
    fs::rename(&tmp_path, &path).with_context(|| {
        format!(
            "failed to rename temporary task file from {} to {}",
            tmp_path.display(),
            path.display()
        )
    })?;
    ...
    match active.len() {
        ...
        n => {
            ...
            let listed = active
                ...
                .join(", ");
            bail!(
                "Found {n} active=true tasks: {listed}. \
                 Run `hotpot task list --json` to inspect, then use `hotpot task stop --all` or \
                 `hotpot task done --task-id <ID>` to converge to a single active task."
            );
        }
    }
}
...
pub(super) fn append_task_line(path: &PathBuf, new_task: &TaskInfo) -> Result<()> {
    let mut line = serde_json::to_string(new_task).with_context(|| {
        format!(
            "failed to serialize task info, task_id: {}",
            new_task.task_id
        )
    })?;
    line.push('\n');
    fs::OpenOptions::new()
        ...
        .with_context(|| format!("failed to append to task file: {}", path.display()))?;
    Ok(())
}
