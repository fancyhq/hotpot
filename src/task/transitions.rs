...
fn mark_task_done_locked(
    ...
    let target_id: String = match task_id {
        Some(id) => {
            ...
            if !exists {
                bail!("未找到 task_id = {id} 的任务，请先运行 `hotpot task list` 确认。");
            }
            id.to_string()
        }
        None => {
            ...
            match active_rows.len() {
                0 => bail!(
                    "没有 active 任务可标记为 Done。可运行 `hotpot task list` 查看现有任务，或显式传 `--task-id`。"
                ),
                ...
                _ => {
                    let listed = active_rows
                        ...
                        .join(", ");
                    bail!(
                        "发现 {n} 条 active=true 的任务：{listed}。请通过 `--task-id` 明确指定要完成的那一条。",
                        n = active_rows.len()
                    );
                }
            }
    ...
    let current = tasks
        ...
        .expect("target_id 必须能在 tasks 中找到，前面已校验存在性");

    if current.status == TaskStatus::Cancelled {
        bail!(
            "任务 {id} 已是 Cancelled，拒绝转为 Done。",
            id = current.task_id
        );
    }
    ...
    {
        bail!(
            "任务 {id} 已是 Done 且 commit={existing}，拒绝覆盖为 {incoming}。",
            id = current.task_id
        );
    }
    ...
    let updated = get_task_list(root_dir, username)?
        .into_iter()
        .find(|task| task.task_id == target_id)
        .ok_or_else(|| anyhow::anyhow!("更新后未能再次定位到 task_id = {target_id}"))?;
    Ok(updated)
}
...
fn mark_task_cancelled_locked(
    ...
    let target_id: String = match task_id {
        ...
        None => {
            ...
            match active_rows.len() {
                0 => bail!(
                    "没有 active 任务可标记为 Cancelled。可运行 `hotpot task list` 查看现有任务，或显式传 `--task-id`。"
                ),
                1 => active_rows[0].task_id.clone(),
                _ => {
                    let listed = active_rows
                        ...
                        .join(", ");
                    bail!(
                        "发现 {n} 条 active=true 的任务：{listed}。请通过 `--task-id` 明确指定要取消的那一条。",
                        n = active_rows.len()
                    );
                }
            }
...
fn attach_worktree_locked(
    ...
    let current = tasks
        ...
        .ok_or_else(|| {
            anyhow::anyhow!("未找到 task_id = {task_id} 的任务，请先运行 `hotpot task list` 确认。")
        })?;

    if current.status != TaskStatus::InProgress {
        bail!(
            "任务 {id} 状态为 {status}，无法挂载 worktree（仅 In Progress 行可挂载）。",
            id = current.task_id,
    ...
    if let Some(existing) = current.worktree_path.as_deref() {
        bail!(
            "任务 {id} 已挂载 worktree：{existing}。请先运行 \
             `hotpot worktree remove --task-id {id}` 清理后再挂。",
            id = current.task_id
        );
    }
...
fn mark_task_resumed_locked(root_dir: &str, username: &str, task_id: &str) -> Result<TaskInfo> {
    ...
    if !exists {
        bail!("未找到 task_id = {task_id} 的任务，请先运行 `hotpot task list` 确认。");
    }
    ...
    let current = tasks
        .iter()
        .find(|task| task.task_id == task_id)
        .expect("task_id 必须能在 tasks 中找到，前面已校验存在性");
